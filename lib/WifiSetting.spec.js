...
var wpaSupplicantContentSimple3 = 	'ctrl_interface=DIR=/var/run/wpa_supplicant GROUP=netdev\n' +
									'update_config=1\n';
...
var wpaSupplicantContentAfterWriteSimple3 = 'ctrl_interface=DIR=/var/run/wpa_supplicant GROUP=netdev\n' +
											'update_config=1\n' +
											'network={\n' +
											'	ssid="New SSID"\n' +
											'	psk="somePassphrase"\n' +
...
describe("WifiSettings", function() {
	...
	describe("writing", function() {
		...
		it("should overwrite the wifi settings if wpa supplicant file contains settings IV", function(done) {
			mockFs({
				"/etc/wpa_supplicant/wpa_supplicant.conf": mockFs.file({content: wpaSupplicantContentSimple3})
			});
			wifiSetting.write({ssid: "New SSID", passphrase: "somePassphrase"}, function(error, data) {
				expect(error).to.equal(null);
				wifiSetting.read(function(error, data) {
					expect(error).to.equal(null);
					expect(data.ssid).to.equal("New SSID");
					expect(fs.readFileSync("/etc/wpa_supplicant/wpa_supplicant.conf", 'utf8')).to.equal(wpaSupplicantContentAfterWriteSimple3);
					done();
				});
			});
		});
	...
	describe("resetting", function() {
		it("should fail to clear the wifi settings if we have no access to wpa supplicant file", function(done) {
			mockFs({
				"/etc/wpa_supplicant/wpa_supplicant.conf": mockFs.file({mode: 000})
			});
			wifiSetting.reset(function(error, data) {
				expect(error).to.deep.equal({error: true, message: "No access to wifi settings"});
				done();
			});
		});

		it("should clear the wifi settings", function(done) {
			mockFs({
				"/etc/wpa_supplicant/wpa_supplicant.conf": mockFs.file({content: wpaSupplicantContentFilled})
			});
			wifiSetting.reset(function(error, data) {
